...
Figure2d_func <- function(dfs_2d){
  ...
  fig2d <- ggplot(dfs3, aes(x=SDG, y=value, fill=group)) +
    ...
    theme_bw() + 
    xlab("SDG indicators") +
    ylab("SDG scores difference\nbetween trade and no-trade scenarios") + 
    ...
    theme(legend.text = element_text(colour = 'black', 
          ...
          legend.position = c(0.9, 0.15))+ # 0.90, 0.95
    
    geom_vline(xintercept=seq(1,length(dfs3[,2])/3,1)+.5,color="gray") +
    geom_hline(yintercept=seq(0,0,0),color="gray") +
    scale_x_discrete(labels=c("SDG99" = "SDGc")) +
  ...
  fname     <- paste0(dir.fig, '/Fig.2d_c_w', wname, '_', timestamp, '.png'); fname
