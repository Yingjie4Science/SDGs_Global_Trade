...
Figure3b_func <- function(sdg.wi){
  ...
  fname = paste(dir.fig, '/Fig.3b', approach, file.name, timestamp, '.jpg', sep = '_')
  
  # png(file=fname,width = 5.5, height = 6, units = "in",res = 300)
  # pdf(file = fname, width = 180, height = 180, paper = 'a4')
  ...
  Fig3b <- ggplot(dt.3b.summary, aes(x    = group_scenario, 
    ...
    geom_errorbar(aes(ymin = value - se, 
                      ymax = value + se),
                  width=0.15, size = 0.15,                   # Width of the error bars
                  position=position_dodge(.8)) +
    #ylim(10,100) +
    scale_y_continuous(breaks=seq(65, 78, 2), # ymax = 81
                       limits=c(65, 78),
                       oob = rescale_none) +
...
# grid.brackets(930, bottom_y, 1570, bottom_y, lwd=1, col="chocolate1")
# dev.off()
# sdg.wi <- sdgc.bygroup.notnerdst.w1
