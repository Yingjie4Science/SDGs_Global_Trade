...
```{r}
indicator1 <- 'Indicator 13.2'

### CO2 emissions in Gg (kt); 1kt = 10^6 kg
### data from WIOD-Environmental_Accounts
loc <- read_excel(df.xlsx,sheet = 7, col_names = T, range = ("U1:AJ42")) # 'S7.net_import_nearby'
...
forestland <- data.frame(forestland[,-1], row.names=forestland[,1])
forestland <- forestland[ order(row.names(forestland)), ]



### CE/FA (gt/ha)
df_rel_c <- (loc + 0     )/(forestland*100)
df_not_c <- (loc + s7.not)/(forestland*100)
df_ner_c <- (loc + s7.dst)/(forestland*100)
df_dst_c <- (loc + s7.ner)/(forestland*100)
...
score_c2 <- data.frame(matrix(NA, nrow = 164, ncol = 18))
...
## Mean SDG score (no average for SDG13)
...
rep.col  <-function(x,n){matrix(rep(x,each=n), ncol=n, byrow=TRUE)}
forestland_base <- as.data.frame(rep.col(forestland[,1], 15)) # set the start year 1995 as base line
forestland_base$group  <- forestland_base$group
names(forestland_base) <- names(forestland_base)
row.names(forestland_base) <- row.names(forestland_base)
forestland <- forestland_base

# df_rel_b <- (loc_base + 0     )/forestland/100; colnames(df_rel_b) <- colnames(df_rel_c)
df_rel_b <- (loc      + 0     )/forestland/100; colnames(df_rel_b) <- colnames(df_rel_c)
df_not_b <- (loc_base + s7.not)/forestland/100; colnames(df_not_b) <- colnames(df_rel_c)
df_ner_b <- (loc_base + s7.dst)/forestland/100; colnames(df_ner_b) <- colnames(df_rel_c)
df_dst_b <- (loc_base + s7.ner)/forestland/100; colnames(df_dst_b) <- colnames(df_rel_c)
...
# df_rel_b <- (loc_base + 0     )/forestland/100; colnames(df_rel_b) <- colnames(df_rel_c)
df_rel_b <- (loc      + 0     )/forestland/100; colnames(df_rel_b) <- colnames(df_rel_c)
df_not_b <- (loc + s7.not.base)/forestland/100; colnames(df_not_b) <- colnames(df_rel_c)
df_ner_b <- (loc + s7.dst.base)/forestland/100; colnames(df_ner_b) <- colnames(df_rel_c)
df_dst_b <- (loc + s7.ner.base)/forestland/100; colnames(df_dst_b) <- colnames(df_rel_c)
