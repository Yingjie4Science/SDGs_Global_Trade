...
### for calculating the difference between dst and ner; by global/ group
...
### 9.4.1.1 CO2 emission/GDP
  CO2 emissions per unit of value added (low carbon intensity indicates high SDG indicator score)
```{r}
indicator1 <- 'Indicator 9.4.1.1'
...
GDP_c_rel <- GDP_c %>% filter(scenario == 'rel_c') %>% dplyr::select(-1, -17) ## remove 1-country-Name; 17-scenario-Name
...
### baseline approach (suggested by one reviewer)
...
### cal CCE/GDP -------------------------------------------------------------------
df_rel_c <- (loc + 0     )/GDP_c_rel
df_not_c <- (loc + s7.not)/GDP_c_not ## 
df_ner_c <- (loc + s7.dst)/GDP_c_ner
df_dst_c <- (loc + s7.ner)/GDP_c_dst

### normalization using the function
score_c1 <- Func_norm_sdg_score_bad(df_rel_c, df_not_c, df_ner_c, df_dst_c,
                                    bottom = 0.05, top = 0.975)
...
fname <- paste0(dir.root, '/data/data_myRData/', 'normalized_score__co2_per_gdp.RData'); fname
...
indicator2 <- 'Indicator 9.4.1.2'

### cal CCE/GDP
df_rel_c <- (loc + 0     ) ## take the log? - log(df_rel_c)
df_not_c <- (loc + s7.not)
df_ner_c <- (loc + s7.dst)
df_dst_c <- (loc + s7.ner)
...
```{r}
lab <- bquote('Virtual CO2 emissions (Gg)')
...
df_rel_b <- (loc      + 0     )/GDP_b_rel; colnames(df_rel_b) <- colnames(df_rel_c)
...
# df_rel_b <- (loc_base + 0     )/GDP_b_rel; colnames(df_rel_b) <- colnames(df_rel_c)
df_rel_b <- (loc      + 0     )/GDP_b_rel; colnames(df_rel_b) <- colnames(df_rel_c)
df_not_b <- (loc + s7.not.base)/GDP_b_not; colnames(df_not_b) <- colnames(df_rel_c)
df_ner_b <- (loc + s7.dst.base)/GDP_b_ner; colnames(df_ner_b) <- colnames(df_rel_c)
df_dst_b <- (loc + s7.ner.base)/GDP_b_dst; colnames(df_dst_b) <- colnames(df_rel_c)
