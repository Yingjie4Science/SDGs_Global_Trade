...
csv_process <- function(file) {
  ...
  s_dst = dt * matrix.distant # data for distant trade scenario
  colnames(s_ner) <- countries     # add col names
  colnames(s_dst) <- countries     # add col names
  s_ner$year <- year_seq
  s_dst$year <- year_seq
  
  ### export data to csv
  csv.name.ner <- paste("csv_result_matrix_ner", file, '.csv' ,sep="_")
  csv.name.dst <- paste("csv_result_matrix_dst", file, '.csv' ,sep="_")
  write.csv(s_ner, file = csv.name.ner)
  write.csv(s_dst, file = csv.name.dst)
  #
  s_ner_sum <- s_ner %>%
    group_by(year) %>%
